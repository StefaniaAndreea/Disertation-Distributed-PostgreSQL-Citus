...
# Utilizare: ./populate_d2.sh <scale_factor>
...
if [ -z "$SF" ] || [ -z "$NW" ]; then
    ...
    echo "Utilizare: ./populate_d2.sh <scale_factor> <nr_workeri>"
    echo "Exemplu:   ./populate_d2.sh sf10 n2"
    echo "           ./populate_d2.sh sf50 n5"
    exit 1
fi
...
echo "Strategia: D2 (colocare per canal)" | tee -a "$LOG_FILE"
...
# PASUL 1: Creare tabele goale
echo "" | tee -a "$LOG_FILE"
echo "--- PASUL 1: Creare tabele goale ---" | tee -a "$LOG_FILE"
...
# PASUL 3: Distributed tables - D2 
echo "" | tee -a "$LOG_FILE"
echo "--- PASUL 3: Creare distributed tables D2 ---" | tee -a "$LOG_FILE"

run_sql "Distributed: store_sales (ancora canal store)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('store_sales','ss_ticket_number');"

run_sql "Distributed: store_returns (colocat cu store_sales)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('store_returns','sr_ticket_number', colocate_with => 'store_sales');"

run_sql "Distributed: web_sales (ancora canal web)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('web_sales','ws_order_number');"

run_sql "Distributed: web_returns (colocat cu web_sales)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('web_returns','wr_order_number', colocate_with => 'web_sales');"

run_sql "Distributed: catalog_sales (ancora canal catalog)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('catalog_sales','cs_order_number');"

run_sql "Distributed: catalog_returns (colocat cu catalog_sales)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('catalog_returns','cr_order_number', colocate_with => 'catalog_sales');"

run_sql "Distributed: inventory (warehouse_sk)" \
    "SET search_path TO $SCHEMA; SELECT create_distributed_table('inventory','inv_warehouse_sk');"
